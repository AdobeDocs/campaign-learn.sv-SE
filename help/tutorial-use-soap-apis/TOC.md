---
user-guide-title: Använd SOAP API:er
user-guide-description: Lär dig hur du använder Adobe Campaign Soap API:er och skapar ett avancerat leveransarbetsflöde.
breadcrumb-title: Använd SOAP API:er
breadcrumb-url: /campaign-learn/use-soap-apis/introduction.html
source-git-commit: c6c343f3087f91fa79291695f99c5878352e4678
workflow-type: tm+mt
source-wordcount: '74'
ht-degree: 0%

---


# Använda Campaign SOAP APIs™ {#use-soap-apis}

+ [Självstudiekurs - använd SOAP API:er](/help/tutorial-use-soap-apis/introduction.md)
+ [Steg 1 - Kom igång med SOAP API:er](/help/tutorial-use-soap-apis/get-started-with-soap-apis.md)
+ [Steg 2 - Lägg till mottagare och artiklar använder SOAP API-begäran](/help/tutorial-use-soap-apis/add-recipients-and-articles-using-soap-api-requests.md)
+ [Steg 3 - Prenumerera användare via SOAP API](/help/tutorial-use-soap-apis/subscribe-users-via-soap-api.md)
+ Steg 4 - Skapa en artikelvarningsleverans{#article-alert-email}
   + [Skapa leverans av artikelaviseringar](/help/tutorial-use-soap-apis/create-article-alert-delivery-overview.md)
   + [Del 1 - Frågeartiklar och mottagarprenumerationsvärden](/help/tutorial-use-soap-apis/query-articles-and-recipient-subscription-values.md)
   + [Del 2 - Skicka en leverans till mottagare som prenumererar](/help/tutorial-use-soap-apis/send-delivery-to-subscribed-recipients.md)
